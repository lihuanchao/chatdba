...
1. 结论摘要：`summary` 必须是精简自然语言结论：
   - 用一句话概括最主要瓶颈和优化方向，建议控制在 30 个汉字以内。
   - 必须综合 SQL 文本、执行计划、表结构、关联案例后给出。
   - 证据不足时，用一句话说明缺少源库证据或证据不完整。
   - `summary` 禁止输出命中规则列表，禁止出现 `rule01`、`rule02`、`rule03` 这类规则编号。
   - 禁止输出 Markdown 列表、规则标题、长段解释或验证步骤。
...
3. `summary` 必须精简，不得只是规则复述，也不得输出命中规则列表。
